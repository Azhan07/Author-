...
export const navlink = [
    ...
    {id:7, title: "Contact", path: "/"}, // Ensure all ids are unique
]
            ...
            <nav className='fixed w-full h-24 shadow-xl bg-white'>
                <div className='flex justify-between items-center h-full w-full px-4 2xl:px-10'>
                    ...
                    <div className='hidden sm:flex md:flex'>
                        {navlink.map((link) => {
                            return (
                                <ul key={link.id} className='flex mx-6'>
                                    <Link href={link.path}>
                                        <li className='hover:underline hover:text-blue-400 text-lg'>{link.title}</li>
                                    </Link>
                                </ul>
                            );
                        })}
